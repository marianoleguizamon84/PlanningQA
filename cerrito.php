...
<div class="screenonly">
  <div id="dwm_header">
    <div id="dwm_areas">
      <div style="font-size: 20px">Sede Cerrito</div>
    </div>
<?php
/*
...
$sql = "SELECT room_name, capacity, id, description FROM $tbl_room WHERE area_id=$area or area_id=21 or area_id=23 ORDER BY area_id, sort_key";
...
<div class=\"screenonly\">
  <div class=\"date_nav\">
    <div class=\"date_before\">
      <a href=\"Cerrito.php?year=$yy&amp;month=$ym&amp;day=$yd&amp;area=$area$room_param\">&lt;&lt;&nbsp;". get_vocab("daybefore") ."
      </a>
    </div>
    <div class=\"date_now\">
      <a href=\"Cerrito.php?area=$area$room_param\">" . get_vocab("gototoday") . "</a>
    </div>
    <div class=\"date_after\">
      <a href=\"Cerrito.php?year=$ty&amp;month=$tm&amp;day=$td&amp;area=$area$room_param\">". get_vocab("dayafter") . "&nbsp;&gt;&gt;
      </a>
    </div>
  </div>
</div>\n";
  ...
  else
  {
    ...
    for ($i = 0; ($row = sql_row_keyed($res, $i)); $i++)
    {
      ...
      // $rooms[] = $row['id'];
      if ($row[id] != 166 AND $row[id] != 179 AND $row[id] != 247){
      $header .= "<th style=\"width: $column_width%\">
                  <a href=\"week.php?year=$year&amp;month=$month&amp;day=$day&amp;area=$area&amp;room=".$row['id']."\"
                  title=\"" . get_vocab("viewweek") . " &#10;&#10;".$row['description']."\">" .
                  htmlspecialchars($row['room_name']) . ($row['capacity'] > 0 ? "(".$row['capacity'].")" : "") . "</th>";
      $rooms[] = $row['id'];
    }
    }
  ...
  else
  {
        ...
        )
    {
      ...
      while (list($key, $room_id) = each($rooms))
      {
        ...
        if ($room_id != 166 AND $room_id != 179 AND $room_id != 247) {
          draw_cell($today[$room_id][$day][$time_t], $query_strings, $cell_class);
        }
      }
