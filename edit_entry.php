...
require_once "classCapacidad.php";
...
if (($room == '157' OR $room == '158' OR $room == '165' OR $room == '166' OR $room == '179' OR $room == '247') AND $permiso < '3'){
  // if ($permiso < '3') {
  	print_header($day, $month, $year, $area, isset($room) ? $room : "");
  	echo '<br>';
  	echo 'No tiene permisos para reservar esta aula<br>';

  	echo 'Por favor contactese con <a href="mailto:erobol@austral.edu.ar?subject=Reserva%20de%20Salón%20Exclusivo%20del%20CAI">Eliana Robol</a> para proceder a realizar reservas en los espacios exclusivos del CAI.<br>';
...
{
  if (document.getElementById("ceco") !== null){
  ...
  if (document.getElementById('cant_alum').value.trim()=="")
  {
    alert("No ha introducido ninguna Cantidad de Alumnos");
    return false;
  }
      ...
      <select id="rooms" name="rooms[]" multiple="multiple" size="5">
        ...
        if ($res)
        {
          for ($i = 0; ($row = sql_row_keyed($res, $i)); $i++)
          {
            ...
            if (($row['id'] != '157' AND $row['id'] != '158' AND $row['id'] != '165' AND $row['id'] != '166' AND $row['id'] != '179' AND $row['id'] != '247') OR $permiso == '3' ) {
